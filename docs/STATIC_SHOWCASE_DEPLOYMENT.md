...
- `docs/COMIC_REAL_RUN_EVIDENCE_TEMPLATE.json` 和公开下载物里的真实运行证据说明必须保留 `asset_identity_cards` 与 `reference_asset_chain`：前者说明每个人物、道具、场景的身份基准图和人工审核状态，后者说明每个镜头引用了哪些已审核资产。没有这两项时，公开页只能证明文件存在，不能证明下游一致性链路成立。
...
- 八份下载物和十五个可复核文件目录，包含 AI 漫剧追溯记录、AI 漫剧生产验收卡、研究办公室阶段性交付声明、研究证据导入模板、`evidence_capture_playbook`、AI 漫剧真实生产声明报告、AI 漫剧真实运行证据收口单、AI 漫剧真实运行证据模板、办公室扩展决策简报、面试官评审包和 GitHub 新用户上手包。
...
它只在手动触发时执行：导出 `dist/public-showcase`、验证静态包、运行敏感信息扫描，然后检查 GitHub Pages 是否已经启用。这个通道不需要 Vercel 授权，也不需要任何 API Key。第一次使用前，需要在 GitHub 仓库 Settings -> Pages 中把 Source 设为 GitHub Actions；如果 Pages 还没启用，workflow 会在构建和扫描通过后显示 `GitHub Pages not enabled` 并跳过部署，避免把外部设置缺口变成反复失败邮件。这种跳过不代表备用公开 URL 已经上线，只能说明 no-key 静态包是好的；此时备用公开入口还没有真的上线。启用 Pages 后重新手动运行 workflow，它才会上传 artifact、部署 GitHub Pages，并访问 `https://atticus-zhou.github.io/sangechoupijiang/` 做线上 URL 验证；只有公开 URL 验证通过，才能分享这个备用入口。
...
- 产品本体仓库的 `Public showcase pages` 失败：它只负责 GitHub Pages 备用公开入口。先运行 `python scripts/diagnose_showcase_workflow_email.py --head-sha <邮件里的 commit> --format markdown`，让脚本核对失败 run、当前 `main` 和本地 workflow 触发方式；也可以用 `--contract-only` 只检查当前 workflow 是否已经手动触发化。若邮件来自旧提交，而且当前 workflow 只有 `workflow_dispatch`，可以把这封失败邮件当作旧发布尝试的残留通知处理。新版本 workflow 会先构建、验证和扫描 no-key 静态包；如果只是 Pages 没启用，会显示 `GitHub Pages not enabled` 并跳过部署，而不是红色失败。它不会证明当前产品不可运行，也不该触发产品代码回滚。
