...
- [ ] 个人网站展示产品定位、办公室大厅、主流程截图、样例交付物和 GitHub 链接。
- [ ] 在线公开版只开放无 Key 演示模式，不消耗作者额度。
- [ ] GitHub README 能让面试官、开发者和普通用户分别看懂怎么体验、怎么运行、怎么扩展。
- [ ] 所有公开展示都不暴露 API Key、Cookie、登录态、用户数据和运行产物。

### 阶段 B：单办公室产品级

目标：先把 AI 漫剧制片办公室做到“可以真实交付制片包”，再考虑新办公室。

- [ ] 用户可从灵感、完整剧本、已有角色设定、参考风格进入工作流。
- [ ] 内阁只负责和人对齐故事，不替三省六部做生产拆解。
- [ ] 三省六部必须产出可审核的故事合同、视觉母版、资产清单、镜头执行卡、提示词包和 Word 制片画布。
- [ ] 用户能在关键节点确认、修改、退回，退回意见必须真实影响下一版结果。
- [ ] 最终 Word 画布能被下游图片/视频工具理解，而不是只适合展示给人看。
...
状态：进行中。AI 漫剧制片办公室与研究办公室已接入固定样例无 Key 演示入口；个人网站仍需继续补充研究样例展示和真实产品截图。
...
- [ ] Continue extending concrete schema validators to research reports and
  future office-specific outputs.
