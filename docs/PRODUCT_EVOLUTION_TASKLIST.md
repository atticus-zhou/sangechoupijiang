...
- [x] 无 Key 演示可以导出为不依赖 FastAPI 后端的静态站点，保留真实产品截图、八份下载物、十一个可复核文件、阅读指南、下游生产 quick-start 和安全边界。
...
- [x] 下载链接必须可用。
  ...
  说明：真实模型跑完后的最终收口必须追加 `python scripts/verify_comic_real_run_evidence_intake.py --latest --format markdown`。它会自动从 `output/workspaces/` 找最新真实工作区制片包，把 Word、manifest、trace、图片证据、刑部视觉质检、兵部提示词谱系和下游交接结论放到同一份 verdict 里，防止页面显示完成但实际交付物不能生产。复核旧项目时，再使用 `--manifest output/你的项目/xxx_handoff_manifest.json` 指向具体文件。
