...
- [x] 无 Key 演示可以导出为不依赖 FastAPI 后端的静态站点，保留真实产品截图、八份下载物、十二个可复核文件、阅读指南、下游生产 quick-start 和安全边界。
...
- [x] 研究办公室成为可靠的人机协作调研工具，重点解决报告、数据、截图证据和来源追溯。
...
- [x] AI 漫剧制片办公室关键按钮会即时显示处理中状态，并说明完成后会停在哪个人工确认点或恢复节点，避免用户误以为“点了没动静”。
...
状态：已形成公开展示包。AI 漫剧制片办公室与研究办公室已接入固定样例无 Key 演示入口；`/api/demo/public-showcase` 已提供 `portfolio_embed`、`release_badge` 和 `public_deployment`，供个人网站或 Vercel 页面复用产品定位、办公室大厅说明、发布状态铭牌、3 分钟演示脚本、主流程截图目标、样例交付物、GitHub 链接和 demo-only 部署边界。
...
- [x] 公开展示清单提供发布状态铭牌，集中说明可公开展示、无 Key、demo-only、真实模型不调用和真实画质未验证。
...
- [x] 视频提示词需要包含原文依据、镜头形式、参考资产、动作链、动作表演、摄影、光线、台词、声音和连续性要求。
- [x] 交付前检查不同资产提示词是否高度雷同，阻止只替换名称的模板化提示词进入正式制片包。
...
- [x] 制片包质量基准会把问题归属到故事、资产、提示词/镜头、图片质检或交付阶段，并提供可执行的部门级恢复动作。
- [x] 图片质量摘要会把每张失败图转换成单图返工卡，包含责任部门、阻塞阶段、优先级、用户提示、建议按钮和操作步骤，避免用户只能看到内部 action。
- [x] 旧版只有 Word、没有 handoff manifest v3 的历史交付明确标记为不可审计，仍可下载但不能冒充质量通过。
...
- [ ] 电商选品办公室：优先候选，先复用研究办公室的数据、竞品、截图计划和 staged claim 边界。
- [ ] 短视频投放办公室：第二候选，等 AI 漫剧素材包和研究洞察更稳定后承接素材投放。
- [ ] 小说或短剧 IP 办公室：第三候选，必须避免和 AI 漫剧制片办公室的内阁/资产拆解职责重叠。
- [ ] 技术项目办公室：暂缓候选，等仓库权限、CI 证据、代码审计和失败回滚协议成熟后再做。

说明：新办公室优先级已写入 `src/offices.py` 的 `future_office_prioritization`，并由 `python scripts/verify_future_office_backlog.py --format markdown`、公开 demo 和静态 showcase 验证。当前判断是“决策已准备，但不启动新办公室”，避免产品再次扩散成一堆半成品入口。
...
- [x] 下载链接必须可用。
  ...
  说明：真实模型跑完后的最终收口必须追加 `python scripts/verify_comic_real_run_evidence_intake.py --latest --format markdown`。它会自动从 `output/workspaces/` 找最新完整可审计的真实工作区制片包，并跳过空 JSON、测试残留和没有图片/资产/镜头/Word 画布的半成品 manifest，把 Word、manifest、trace、图片证据、刑部视觉质检、兵部提示词谱系和下游交接结论放到同一份 verdict 里，防止页面显示完成但实际交付物不能生产。复核旧项目时，再使用 `--manifest output/你的项目/xxx_handoff_manifest.json` 指向具体文件。
