...
状态：暂缓。当前阶段先不做演示模式，优先把真实本地产品的模型预检、生产链路和交付物稳定下来。

- [ ] 首页增加清晰的“演示模式”入口。
- [ ] 演示入口包含 `体验AI漫剧制片办公室`。
- [ ] 演示入口包含 `体验研究办公室`。
- [ ] 演示模式加载固定项目、固定流程状态、固定产物和固定下载文件。
- [ ] 演示模式明确提示“不消耗 API Key，不调用真实模型”。
- [ ] 演示模式禁止误触真实生产接口。

验收标准：

- [ ] 用户打开网页后 1 分钟内能看到产品价值。
- [ ] 不填写任何 API Key 也能查看完整流程和交付物。
- [ ] 演示项目能展示 Agent 分工、人工审核节点和最终交付文件。
...
说明：无 Key 演示模式仍按当前阶段决策暂缓；真实本地产品 readiness 由 `src/product_readiness.py` 审计，并由 `tests/test_comic_production_readiness.py` 验证。
