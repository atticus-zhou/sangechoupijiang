...
1. 打开个人网站或本地首页，进入公开展示页。
2. 查看 `/api/demo/public-showcase` 对应的产品定位、访客路径、推荐 demo 和安全边界。
...
这些命令会检查公开演示、样例交付、下游制片交接、研究办公室边界、总发布门禁和敏感信息扫描。通过后，再复制 `config.example.yaml` 为 `config.yaml`，进入本地真实模式，用自己的模型 Key 测试各部门。
