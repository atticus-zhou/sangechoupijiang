...
class ComicV2ProductionBenchmarkVerifierTests(unittest.TestCase):
    def test_json_reports_honest_fixture_quality_claim(self):
        ...
        by_asset_type = payload["image_quality_summary"]["by_asset_type"]
        ...
        self.assertEqual(payload["real_model_evidence_requirements"]["status"], "evidence_missing")
        self.assertFalse(payload["real_model_evidence_requirements"]["ready_for_real_quality_claim"])
        self.assertIn("non_fixture_images", payload["real_model_evidence_requirements"]["missing_check_ids"])
        self.assertIn("provider_model_bound", payload["real_model_evidence_requirements"]["missing_check_ids"])
    ...
    def test_markdown_is_readable_and_discloses_fixture_limit(self):
        ...
        self.assertIn("Image Quality By Asset Type", completed.stdout)
        self.assertIn("| character | 2 | 2 | 0 | 0% | - |", completed.stdout)
        self.assertIn("| prop | 2 | 2 | 0 | 0% | - |", completed.stdout)
        self.assertIn("| scene | 3 | 3 | 0 | 0% | - |", completed.stdout)
        self.assertIn("Real Model Evidence Requirements", completed.stdout)
        self.assertIn("Ready for real quality claim: `False`", completed.stdout)
