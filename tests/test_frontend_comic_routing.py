...
class FrontendComicRoutingTests(unittest.TestCase):
    ...
    def test_history_renders_delivery_summary(self):
        ...
        self.assertIn("primary_download_uri", js)
        self.assertIn("file_download_uri", js)
        self.assertIn("打开文件", js)
        self.assertIn("下载记录", js)
        self.assertIn("history-artifact-links", js)
        self.assertIn("function officeLabelForHistory", js)
        self.assertIn("function historyVisualReviewLabel", js)
        self.assertIn("function historyPackageQualityLabel", js)
        self.assertIn("officeLabelForHistory(h.office_id)", js)
        self.assertIn("officeLabelForHistory(item.office_id)", js)
        self.assertIn("AI漫剧制片办公室", js)
        self.assertNotIn("`${h.office_id}办公室`", js)
    ...
    def test_hall_renders_public_showcase_and_no_key_demo_entry(self):
        ...
        self.assertIn("demo.deliverable_reading_guide", js)
        self.assertIn("demo.evidence_handoff", js)
        self.assertIn("待补证据交接表", js)
        self.assertIn("item.target_evidence", js)
        self.assertIn("item.upgrades", js)
        ...
        self.assertIn(".research-handoff-grid", css)
    ...
    def test_public_showcase_page_renders_manifest_for_portfolio_visitors(self):
        ...
        self.assertIn("prompt_quality_summary?.checks", js)
        self.assertIn("prompt-gate-checks", js)
        self.assertIn("提示词门禁", js)
        ...
        self.assertIn("portfolio.first_run_paths", js)
        self.assertIn("firstRunPaths", js)
        self.assertIn("第一次拿到项目怎么走", js)
        self.assertIn("public-first-run-grid", js)
        self.assertIn("portfolio.reproducibility_checklist", js)
        self.assertIn("portfolio.shot_contract", js)
        self.assertIn("shotContractFields", js)
        self.assertIn("public-shot-contract-grid", js)
        self.assertIn("portfolio.release_badge", js)
        self.assertIn("public-release-badge", js)
        self.assertIn("releaseBadge.primary_gate", js)
        self.assertIn("claimUpgradeChecklist", js)
        self.assertIn("realProductionClaim.claim_upgrade_checklist", js)
        self.assertIn("claimUpgradeRecovery", js)
        self.assertIn("realProductionClaim.claim_upgrade_recovery", js)
        self.assertIn("public-claim-upgrade", js)
        self.assertIn("public-claim-recovery", js)
        self.assertIn("3 分钟演示脚本", js)
        self.assertIn("复现与验收清单", js)
        self.assertIn("item.command", js)
        self.assertIn("item.expected", js)
        self.assertIn("item.if_fails", js)
        self.assertIn("item.visitor_action", js)
        self.assertIn("item.product_response", js)
        self.assertIn("item.proof", js)
        self.assertIn("item.boundary", js)
        ...
        self.assertIn(".prompt-gate-checks", css)
        self.assertIn(".public-audience-grid", css)
        self.assertIn(".public-interview-script", css)
        self.assertIn(".public-first-run-grid", css)
        self.assertIn(".public-repro-list", css)
        self.assertIn(".public-release-badge", css)
        self.assertIn(".public-claim-upgrade", css)
        self.assertIn(".public-claim-recovery", css)
        self.assertIn(".public-shot-contract-grid", css)
    ...
    def test_confirm_story_button_has_visible_loading_and_error_handling(self):
        ...
        self.assertIn("stage: 'visual_bible_review'", confirm_fn)
        self.assertIn("确认故事与视觉母版入口", confirm_fn)
        self.assertIn("随后生成故事合同和视觉母版", confirm_fn)
    ...
    def test_comic_workbench_renders_latest_real_run_audit_card(self):
        ...
        self.assertIn("downstream_decision", render_fn)
        self.assertIn("handoff_decision_label", render_fn)
        self.assertIn("recommended_user_action", render_fn)
        ...
        self.assertIn("handoff_evidence_checklist", render_fn)
        self.assertIn("required_asset_types", render_fn)
        self.assertIn("required_image_roles", render_fn)
        self.assertIn("audit_command", render_fn)
        self.assertIn("evidence_template_uri", render_fn)
        self.assertIn("下载证据模板", render_fn)
        self.assertIn("交付证据", render_fn)
        self.assertIn("审计标准", render_fn)
        ...
        self.assertIn("trace_uri", render_fn)
    ...
    def test_comic_workbench_renders_runtime_status_panel(self):
        ...
        self.assertIn("claimLabel", js)
        self.assertIn("claimValue", js)
        self.assertIn("wordDownloadLabel", js)
        self.assertIn("handoffDownloadLabel", js)
        self.assertIn("downloads.word_canvas_label", js)
        self.assertIn("downloads.handoff_manifest_label", js)
        self.assertIn("recovery_action", js)
        self.assertIn("renderRecoveryPlaybook(recovery)", js)
        self.assertIn("recoverComicV2Quality", js)
    ...
    def test_history_renders_comic_v2_prompt_quality_trace(self):
        ...
        self.assertIn("historyPackageQualityLabel(summary.package_quality_claim)", summary_fn)
        self.assertIn("legacy_unverifiable", js)
        self.assertIn("const benchmark = trace.quality_benchmark || {}", trace_fn)
        self.assertIn("const productionEvidence = trace.image_production_evidence || {}", trace_fn)
        self.assertIn("productionEvidence.usable_image_count", trace_fn)
        ...
        self.assertIn("history-production-evidence", trace_fn)
        ...
        self.assertIn("usage_contract", js)
        self.assertIn("reference_policy", js)
        self.assertIn("用途合同", js)
    ...
    def test_quality_recovery_button_routes_to_the_responsible_stage(self):
        ...
        self.assertIn("renderRecoveryPlaybook(recovery)", actions)
        self.assertIn("function renderRecoveryPlaybook", js)
        self.assertIn("operator_steps", js)
        self.assertIn("expected_stage", js)
        self.assertIn("preserves", js)
        self.assertIn("clears", js)
        self.assertIn("renderRecoveryPlaybook(action)", js)
        self.assertIn("renderRecoveryPlaybook(benchmark.recommended_recovery)", js)
