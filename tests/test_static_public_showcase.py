...
class StaticPublicShowcaseTests(unittest.TestCase):
    ...
    def test_export_is_backend_free_and_contains_real_downloads(self):
        ...
        self.assertEqual(summary["download_count"], 8)
        ...
        self.assertEqual(showcase["visitor_acceptance_guide"]["download_count"], 12)
        ...
        self.assertEqual(len(visitor_guide["download_acceptance"]), 12)
        ...
        reviewer_fallback = showcase["portfolio_embed"]["reviewer_fallback_packet"]
        ...
        self.assertEqual(showcase["public_deployment"]["reviewer_fallback_packet"]["status"], reviewer_fallback["status"])
        self.assertEqual(deploy_manifest["reviewer_fallback_packet"]["status"], reviewer_fallback["status"])
        self.assertEqual(visitor_guide["reviewer_fallback_packet"]["status"], reviewer_fallback["status"])
        ...
        self.assertEqual(showcase["static_export"]["reviewable_file_count"], 12)
        ...
        self.assertIn("八份公开下载物已经随静态站点一起导出", script_text)
        ...
        self.assertIn("8 个下载物", repro[2]["expected"])
        self.assertIn("9 个可复核文件", repro[2]["expected"])
        ...
        model_setup = showcase["portfolio_embed"]["model_setup_guide"]
        ...
        self.assertIn("常见误填", json.dumps(model_offices["comic_production"], ensure_ascii=False))
        ...
        self.assertEqual(deploy_manifest["sample_download_count"], 8)
        ...
        latest_audit_card = runtime_by_office["comic_production"]["latest_real_run_audit_card"]
        self.assertEqual(latest_audit_card["title"], "最新真实制片包审计")
        self.assertEqual(latest_audit_card["endpoint"], "/api/comic-production/latest-real-run-audit")
        self.assertFalse(latest_audit_card["requires_api_key"])
        self.assertFalse(latest_audit_card["calls_real_models"])
        self.assertFalse(latest_audit_card["writes_workspace"])
        self.assertIn("status=no_auditable_manifest", latest_audit_card["empty_state"])
        self.assertIn("Word 画布", latest_audit_card["ready_state"])
        self.assertIn("真实质量必须", latest_audit_card["safe_public_claim"])
        ...
        self.assertIn("renderLatestRealRunAuditCard", static_script)
        self.assertIn("latest_real_run_audit_card", static_script)
        self.assertIn("portfolio.runtime_acceptance_summary", static_script)
        self.assertIn("runtime-acceptance-card", style_text)
        self.assertIn("latest-real-run-audit-preview", style_text)
        ...
        self.assertIn("renderModelSetupGuide", static_script)
        self.assertIn("portfolio.model_setup_guide", static_script)
        self.assertIn("first-run-paths", index_text)
        self.assertIn("first-run-guide-checks", index_text)
        self.assertIn("model-setup-guide", index_text)
        self.assertIn("first-run-grid", style_text)
        self.assertIn("model-setup-grid", style_text)
        ...
        self.assertIn("reviewer_fallback_packet", static_script)
        self.assertIn("离线评审包", static_script)
        self.assertIn("reviewer-fallback-card", style_text)
        self.assertIn("fallback-command-list", style_text)
    ...
    def test_static_readiness_verifier_is_public_operator_readable(self):
        ...
        self.assertIn("Downloadable deliverables: 8", completed.stdout)
        self.assertIn("Reviewable catalog: 12 files", completed.stdout)
        self.assertIn("Visitor acceptance guide: 8 steps / downloads=12 / live=external_required", completed.stdout)
        self.assertIn("Reviewer fallback packet: available_when_live_route_stale / commands=3 / archive=tmp/three-cobblers-reviewer-packet.zip", completed.stdout)
        self.assertIn("Handoff recovery inventory: 12 items / actions=regenerate_images", completed.stdout)
        self.assertIn("Handoff asset type quality: character=24/24, rework=0; prop=24/24, rework=0; scene=36/36, rework=0", completed.stdout)
        self.assertIn("Fast review route: 5/5", completed.stdout)
        self.assertIn("Reading guide: 12/12", completed.stdout)
        ...
        self.assertIn("Model setup guide: 2 offices / no-key view", completed.stdout)
