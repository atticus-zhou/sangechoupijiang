...
class ComicV2DownstreamHandoffVerifierTests(unittest.TestCase):
    ...
    def test_fixture_package_is_downstream_handoff_ready(self):
        ...
        self.assertEqual(result["image_file_fingerprints_verified"], 7)
        self.assertEqual(result["image_file_fingerprint_mismatches"], 0)
    ...
    def test_cli_json_exposes_downstream_readiness(self):
        ...
        self.assertEqual(payload["image_file_fingerprints_verified"], payload["image_count"])
        self.assertEqual(payload["image_file_fingerprint_mismatches"], 0)
    ...
    def test_existing_manifest_reports_image_fingerprint_mismatch(self):
        module = self._module()
        with tempfile.TemporaryDirectory() as tmp:
            delivery = verify_delivery(FIXTURE, Path(tmp))
            manifest_path = Path(delivery["handoff_manifest_path"])
        ...
        self.assertEqual(result["status"], "failed")
        self.assertFalse(result["downstream_handoff_ready"])
        self.assertEqual(result["image_file_fingerprint_mismatches"], 1)
        self.assertIn("file_sha256 does not match image bytes", "\n".join(result["errors"]))
    ...
    def test_cli_markdown_is_readable_for_reviewers(self):
        ...
        self.assertIn("Image file fingerprints verified: 7/7", completed.stdout)
