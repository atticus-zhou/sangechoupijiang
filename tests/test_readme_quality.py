...
class ReadmeQualityTests(unittest.TestCase):
    def test_readme_is_readable_chinese_and_current_for_comic_v2(self):
        ...
        self.assertIn("python scripts/diagnose_showcase_workflow_email.py --contract-only --format markdown", text)
    ...
    def test_readme_describes_office_protocols_and_launch_gates(self):
        ...
        self.assertIn("交付验收", text)
        self.assertIn("用户下一步", text)
        self.assertIn("人能看懂的交付验收卡", text)
        self.assertIn("结构可交接，真实画质未验证", text)
    ...
    def test_readme_gives_new_user_model_setup_path(self):
        ...
        self.assertIn("GET /api/comic-production/latest-real-run-audit", text)
        self.assertIn("status=no_auditable_manifest", text)
        self.assertIn("missing_checks", text)
        self.assertIn("safe_public_claim", text)
        self.assertIn("最新真实制片包审计", text)
        self.assertIn("它不跟随当前下拉选中的项目盲目变化", text)
        self.assertIn("Word 画布和引用清单下载入口", text)
        self.assertIn("Image files present: x/y", text)
        self.assertIn("images[].file", text)
        self.assertIn("首帧参考图和资产引用链都能解析到真实图片", text)
        self.assertIn("prompt_strategy_lineage.ready_for_real_quality_claim=True", text)
        self.assertIn("提示词策略追溯", text)
    ...
    def test_readme_model_table_matches_comic_production_roles(self):
        ...
        self.assertIn("| 刑部 | 视觉理解模型 |", text)
        self.assertIn("不要把普通文本模型填到这里期待它看图", text)
    ...
    def test_readme_documents_public_demo_deployment_and_safety(self):
        ...
        self.assertIn("asset_identity_cards", text)
        self.assertIn("reference_asset_chain", text)
        self.assertIn("每个人物、道具、场景都必须有身份基准图和人工审核状态", text)
        self.assertIn("每个镜头都必须能追溯到已审核资产", text)
    ...
    def test_readme_documents_public_asset_requirement_matrix(self):
        ...
        self.assertIn("source_mismatch_or_stale_vercel_bundle", text)
