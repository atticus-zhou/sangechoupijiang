...
class PublicShowcaseManifestTests(unittest.TestCase):
    def test_public_showcase_manifest_packages_product_story_and_demo_links(self):
        ...
        self.assertGreaterEqual(len(embed["deliverable_reading_guide"]), 9)
        self.assertEqual([item["order"] for item in embed["deliverable_reading_guide"]], [1, 2, 3, 4, 5, 6, 7, 8, 9])
        ...
        self.assertTrue(any("资产使用地图" in item["title"] for item in embed["deliverable_reading_guide"]))
        self.assertTrue(any("追溯记录" in item["title"] for item in embed["deliverable_reading_guide"]))
        self.assertTrue(any("生产验收卡" in item["title"] for item in embed["deliverable_reading_guide"]))
        ...
        reading_guide_text = json.dumps(embed["deliverable_reading_guide"], ensure_ascii=False)
        self.assertIn("accepted_for_real_downstream", reading_guide_text)
        self.assertIn("asset_usage_map", reading_guide_text)
        self.assertIn("identity_baseline_image", reading_guide_text)
        self.assertIn("referenced_shots", reading_guide_text)
        ...
        acceptance = embed["production_acceptance"]
        self.assertEqual(acceptance["uri"], "/api/demo/comic-production/production-acceptance")
        self.assertEqual(acceptance["status"], "passed")
        self.assertTrue(acceptance["accepted_for_public_demo"])
        self.assertFalse(acceptance["accepted_for_real_downstream"])
        self.assertEqual(acceptance["downstream_status"], "structure_demo_only")
        self.assertEqual(acceptance["failure_count"], 0)
        self.assertTrue(any(item["id"] == "real_quality_boundary" for item in acceptance["checklist"]))
        self.assertIn("结构演示", acceptance["human_decision"])
        acceptance_response = client.get("/api/demo/comic-production/production-acceptance")
        self.assertEqual(acceptance_response.status_code, 200)
        acceptance_payload = acceptance_response.json()
        self.assertFalse(acceptance_payload["requires_api_key"])
        self.assertFalse(acceptance_payload["calls_real_models"])
        self.assertFalse(acceptance_payload["writes_workspace"])
        self.assertNotIn("E:\\", json.dumps(acceptance_payload, ensure_ascii=False))
        ...
        self.assertIn("8 个下载物", reproducibility[2]["expected"])
        self.assertIn("9 个可复核文件", reproducibility[2]["expected"])
        self.assertIn("9/9 阅读指南", reproducibility[2]["expected"])
