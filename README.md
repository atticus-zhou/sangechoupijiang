...
三个臭皮匠是一个本地优先的多 Agent 协作工作台。它把复杂任务拆成不同“办公室”，再由多个分工明确的 Agent 协同完成，让用户拿到可审核、可追踪、可下载的交付物，而不是只拿到一段聊天回复。
...
8. 输出页面式 Word 制片画布，给下游视频生成或剪辑平台使用。
...
成功时会输出 `V2 delivery verified`，并在本地 `output/comic_v2_verification/` 生成测试用 Word 制片画布。`output/` 已被忽略，不会提交到 GitHub。
...
如果要公开部署，不要把自己的 API Key 暴露给访问者。更安全的方式是让使用者填写自己的 Key，或者只提供演示模式。
