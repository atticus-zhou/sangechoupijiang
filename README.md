...
第一次从 GitHub 下载项目，先看 [START_HERE.md](START_HERE.md)。它会用最短路径告诉你：只是看 demo、本地真实使用、公开部署、继续开发新办公室，分别应该先做什么，以及什么时候才需要 API Key。
...
历史页可以下载 Word 画布，也可以查看制片追溯：故事版本、风格版本、资产版本、提示词数量、视觉质检结果和交付审计。追溯 JSON 还会暴露 `image_production_evidence` 和 `image_quality_summary`，用来区分当前图片证据是固定样例、缺图、部分真实模型，还是已经通过真实模型视觉质检；同时记录可用图、废片/返工图、返工率、失败图片 ID 和 `rework_instructions`。每条返工指令都会说明责任部门、卡在哪个阶段、优先级、给用户看的原因、建议按钮和操作步骤，避免用户只看到一个内部 action 却不知道下一步该做什么。恢复动作会优先读取 `image_quality_summary.failed_image_ids`，只重生问题图片并复审；没有失败图 ID 时，才把恢复动作当作真实模型证据升级计划处理，避免为了几张废片推翻已确认的故事、资产和提示词。
...
Windows 上这条命令会先检查依赖和本地配置，再运行 `doctor.py`，最后启动 `http://127.0.0.1:8080/`。如果端口被占用，可以指定端口，例如 `.\start_windows.bat 8081`。只想检查启动条件、不启动服务器时，运行 `.\start_windows.bat --check`。
...
python scripts/diagnose_showcase_workflow_email.py --contract-only --format markdown
...
如果邮箱又收到 `Public showcase pages` 失败，不要凭邮件标题判断产品坏了。先运行 `python scripts/diagnose_showcase_workflow_email.py --head-sha <邮件里的 commit> --format markdown`；它会核对当前 workflow 是否只剩 `workflow_dispatch`、邮件里的失败 run 是否来自旧提交、以及下一步应该处理 Pages 设置、Vercel 线上 404，还是产品本体 release gate。
...
7. 如果历史追溯显示图片证据仍是 fixture、缺图或未完整质检，不要把它当成真实画质样例；先看 `image_quality_summary` 里的废片/返工数量、失败图片 ID 和 `rework_instructions`。每张问题图都会被归类为“补跑视觉质检”“保留提示词重新生图”或“退回提示词重写”，再按历史页推荐的恢复动作补跑真实图片和视觉质检。恢复请求会携带目标图片 ID，优先只处理这些问题图，不要求用户整包重跑。
...
| 刑部 | 视觉理解模型 | 读取生成图片，检查人物一致性、画风漂移、道具错误和交付风险；不要把普通文本模型填到这里期待它看图 |
...
```powershell
python scripts/verify_research_evidence_intake.py --input output/your_project/research_evidence.json --strict-real-values --format markdown
```
...
- 产品仓库里的 `Public showcase pages` 失败：这是 GitHub Pages 备用入口，不是本地真实服务。先运行 `python scripts/diagnose_showcase_workflow_email.py --head-sha <邮件里的 commit> --format markdown`；它会比对邮件里的 commit SHA、当前 `main` 和 `.github/workflows/pages-showcase.yml` 的触发方式。如果失败发生在旧提交，且当前 workflow 只有 `workflow_dispatch`，说明它不会再随 push 自动失败发信。新版本 workflow 如果只看到 `GitHub Pages not enabled`，表示 no-key 静态包已验证但 Pages 设置没开；要真正使用这个备用入口，再到 Settings -> Pages 把 Source 设为 GitHub Actions 后手动触发。
- 本地 CI 通过但 `/three-stooges/` 还是 404：这通常是 Vercel 还在服务旧部署，或者 Vercel 授权没有完成；继续看个人网站的 `npm run doctor:deploy` 和 `npm run check:online`，不要改产品代码去掩盖 404。如果 `npm run check:online:json` 报告 `deployment_diagnosis.status = "source_mismatch_or_stale_vercel_bundle"`，说明生产首页能打开，但 `/build-info.json`、`/three-stooges/` 和线上 bundle 没有对应当前个人网站构建；如果报告 `deployment_diagnosis.status = "stale_vercel_build_by_curl_probe"`，说明 Node 请求被连接重置，但 curl 已证明首页 200、`/build-info.json` 和 `/three-stooges/` 404，不能进一步宣称已经检查过 live JS bundle。两种情况都优先去 Vercel Dashboard 核对 Git repository、production branch、Root Directory、Build Command 和 Output Directory。
