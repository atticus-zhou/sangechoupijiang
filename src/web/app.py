...
from fastapi import FastAPI, WebSocket, WebSocketDisconnect, HTTPException, Request, UploadFile, File, Form, Response, Query
...
def _build_health_payload() -> dict:
    ...
    return {
        ...
        "legacy_policy": "旧 AI 漫剧办公室只作为兼容入口；公开大厅默认进入 comic_production。",
        "checks": {
            "offices": "/api/offices",
            "offices_with_legacy": "/api/offices?include_legacy=true",
...
def _build_first_run_guide_payload() -> dict:
    return {
        "status": "ready",
        ...
        "calls_real_models": False,
        "title": "第一次使用应该先做什么",
        "summary": "先确认服务活着，再按目标选择无 Key 演示、本地真实使用或开发者扩展。",
        "primary_next_action": "如果只是想快速看产品，请先点无 Key 演示；如果要真实生产，再去模型页面逐个部门测试。",
        "paths": [
            {
                "id": "public_demo",
                "label": "先看无 Key 演示",
                "fit_for": "面试官、访客、第一次打开项目的人",
                "requires_model_credentials": False,
                "calls_real_models": False,
                "first_actions": [
                    "打开大厅里的 AI 漫剧无 Key 演示",
                    "下载 Word 画布和 handoff manifest",
                    "阅读真实生产声明，确认当前公开样例只证明结构和交付链路",
                ],
                "evidence": [
            ...
            },
            {
                "id": "local_real_use",
                "label": "配置本地真实使用",
                "fit_for": "想让办公室调用自己模型产出真实结果的使用者",
                "requires_model_credentials": True,
                "calls_real_models": True,
                "first_actions": [
                    "复制 config.example.yaml 为 config.yaml",
                    "在模型页面按办公室和部门填写 API Key",
                    "先测试每个部门，再回到工作台生产",
                ],
                "evidence": [
                    "/api/system/preflight",
                    "/api/offices/comic_production/preflight",
                    "/api/offices/comic_production/real-production-readiness",
                ],
            },
            {
                "id": "developer_extension",
                "label": "开发新办公室",
                "fit_for": "想新增短视频、选品、故事 IP 或技术项目办公室的开发者",
                "requires_model_credentials": False,
                "calls_real_models": False,
                "first_actions": [
                    "先看办公室协议，不直接复制旧页面",
                    "确认模型配置、工作区、历史和产物全部隔离",
                    "补齐无 Key demo、schema gate、恢复动作和发布门禁",
                ],
                "evidence": [
        ...
        "quick_checks": [
            ...
            },
            {
                "id": "local_doctor",
                "label": "本地环境自检",
                "command": "python scripts/doctor.py --format markdown",
            },
            {
                "id": "model_guidance",
                "label": "模型配置说明",
                "command": "python scripts/verify_model_configuration_guidance.py --format markdown",
            },
            {
                "id": "onboarding_packet",
                "label": "导出 GitHub 入门包",
                "command": "python scripts/export_github_onboarding_packet.py --format markdown",
            },
            {
                "id": "release_gate",
                "label": "公开发布总门禁",
        ...
        "troubleshooting": [
            ...
            {
                "symptom": "真实生产卡在模型调用前",
                "check": "/api/offices/comic_production/preflight",
                "next_action": "回到模型页面，按缺失部门测试 API Key。",
            },
            {
                "symptom": "不知道公开展示能不能给别人看",
                "check": "python scripts/verify_release_readiness.py --format markdown",
                "next_action": "只有 no-key release gate 通过后再公开展示。",
            },
        ],
    }
...
@app.get("/api/first-run-guide")
async def first_run_guide_api():
    """Return a no-key first-run guide for the office hall and GitHub users."""
    return _build_first_run_guide_payload()
...
@app.get("/api/offices")
async def get_offices(include_legacy: bool = Query(False)):
    """List available office profiles."""
    return {"offices": list_offices(include_legacy=include_legacy)}
...
def _public_showcase_deliverable_reading_guide() -> list[dict]:
    return [
        ...
        {
            "order": 3,
            "title": "再看 AI 漫剧资产使用地图",
            "uri": "/api/demo/comic-production/files/handoff_manifest.json",
            "look_for": "asset_usage_map 中每个资产的 identity_baseline_image、image_roles、referenced_shots、downstream_instruction 和 handoff_ready 是否完整。",
        ...
        "asset_types": [
            {
                ...
                "must_not_do": [
                    "禁止把人物放进完整剧情场面。",
                    "禁止添加复杂背景、战斗特效、随机文字、随机 logo 或其他角色。",
                    "禁止让每张基础图变成不同画风的单幅插画。",
                ],
    ...
    return [
        {
            ...
            "suggested_file_name": "evidence_01_feigua_price_band.png",
            "acceptance": "截图能看清平台、榜单或商品页、价格区间、时间范围；敏感账号信息已遮挡。",
        ...
        },
        {
            ...
            "suggested_file_name": "evidence_02_feigua_competitor_ranking.png",
            "acceptance": "截图至少能对应品牌/商品名、排行或销量热度字段、榜单范围；无法确认的数据必须标记待核验。",
        ...
        },
        {
            ...
            "suggested_file_name": "evidence_03_review_pain_points.png",
            "acceptance": "截图能看清评论原文、商品或内容对象、评论时间或排序依据；不得把个别评论写成已验证总体结论。",
...
def _research_demo_evidence_gap_cards(evidence_handoff: list[dict]) -> list[dict]:
    ...
    for index, item in enumerate(evidence_handoff, start=1):
        ...
        cards.append({
            ...
            "file_name": item.get("suggested_file_name") or f"evidence_{index:02d}_{item.get('id', 'research_gap')}.png",
            "acceptance": item.get("acceptance") or "截图能看清页面主题、平台、时间范围和关键数字；敏感账号信息已遮挡。",
...
async def get_public_showcase_demo_api():
    ...
    return {
        ...
        "portfolio_embed": {
            ...
            "model_setup_guide": build_model_setup_guide(),
                ...
                "evidence_handoff": [
                    {
                        ...
                        "suggested_file_name": str(item.get("suggested_file_name") or ""),
