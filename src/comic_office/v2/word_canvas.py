...
def _add_agent_handoff_summary(doc: Document) -> None:
    ...
    _add_callout(
        doc,
        "阅读方式",
        "每一行代表一个办公室交接点。上游产物满足验收后，才交给下一步继续生产；人工审核点不能被后续 Agent 跳过。",
        fill=SILVER,
        accent=INDIGO,
        compact=True,
    )
    _add_label_detail(doc, [
        ("内阁 / 中书省", "负责：锁定故事合同与风格圣经；交给：资产拆解；验收：故事原文、故事版本、画风、时代和禁用元素齐全。"),
        ("中书省 / 门下省", "负责：拆出人物、道具、场景并审校；交给：提示词与镜头执行包；验收：每个资产都有原文证据、故事用途、计划图片和审核状态。"),
        ("尚书省", "负责：调度阶段和恢复动作；交给：下一责任部门；验收：当前阶段、阻塞原因、下一步动作和责任部门可追踪。"),
        ("吏部 / 户部", "负责：维护资产身份证、版本和引用台账；交给：工部、兵部；验收：人物、道具、场景 ID 稳定，图片和镜头引用不串版本。"),
        ("兵部 / 刑部", "负责：生成镜头执行卡并检查提示词；交给：基础图片生产；验收：提示词引用已审核资产，动作链、台词、光影和负面提示词单独成段。"),
        ("工部 / 刑部", "负责：生成基础资产图并逐张质检；交给：Word 画布交付；验收：人物和道具基础图保持干净白底，场景图保留空间信息，风险项有处理结论。"),
        ("礼部 / 刑部", "负责：组装 Word 画布、引用清单和审计结论；交给：下游视频平台；验收：Word、图片、镜头卡和 handoff manifest 可下载且引用一致。"),
    ], compact=True)
