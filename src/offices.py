    ...
    return {
        ...
        "future_office_candidates": [
            ...
            },
            {
                ...
                "first_schema_contracts": [
                    {"schema_id": "selection_decision_report", "owner_agent": "gongbu", "required_fields": ["category", "decision", "confidence", "key_reasons", "evidence_gap_ids"], "acceptance": "选品结论必须能回到证据缺口和关键原因，不能只给一句推荐。"},
                    {"schema_id": "competitor_price_band", "owner_agent": "hubu", "required_fields": ["competitor", "platform", "price_low", "price_high", "source_or_gap"], "acceptance": "价格带必须标明平台和来源；缺截图时写入待补证据。"},
                    {"schema_id": "review_pain_point_table", "owner_agent": "hubu", "required_fields": ["pain_point", "review_signal", "affected_user", "opportunity", "source_or_gap"], "acceptance": "评论痛点必须连接用户类型、机会点和来源或缺口。"},
                    {"schema_id": "evidence_gap_cards", "owner_agent": "xingbu", "required_fields": ["gap_id", "target_page", "why_needed", "suggested_file_name", "acceptance"], "acceptance": "每个待补证据都要说明页面、原因、命名和验收标准。"},
                ],
                "first_recovery_events": [
                    {"stage": "evidence_capture", "action": "sync_selection_evidence", "preserves": ["category_scope", "uploaded_screenshots"], "clears": ["selection_decision_report", "competitor_price_band", "review_pain_point_table"], "user_message": "补充截图后只重跑证据识别和选品表格，不清空品类范围。"},
                    {"stage": "decision_packaging", "action": "rebuild_selection_pack", "preserves": ["category_scope", "source_list", "competitor_price_band", "review_pain_point_table"], "clears": ["selection_decision_report", "public_claim_report"], "user_message": "结论不可信时重建选品决策包，保留已核验来源和表格。"},
                ],
