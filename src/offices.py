    ...
    recovery_actions=[
        {
            ...
            "preserves": ["research_scope", "uploaded_screenshots", "workspace_history"],
            "clears": ["extracted_evidence", "draft_report", "quality_report"],
        },
        {
            ...
            "preserves": ["research_scope", "uploaded_screenshots", "source_notes"],
            "clears": ["extracted_evidence", "evidence_gap_cards", "draft_report"],
        },
        {
            ...
            "preserves": ["research_scope", "source_list", "uploaded_screenshots"],
            "clears": ["standard_report", "briefing", "quality_report"],
        },
        {
            ...
            "method": "POST",
            "path_template": "/api/tasks/{task_id}/recover-artifacts",
            "preserves": ["research_scope", "source_list", "data_table", "competitor_table"],
            "clears": ["standard_report", "briefing", "quality_report"],
        },
    ],
    acceptance_criteria=[
...
def list_office_extension_blueprint() -> dict:
    ...
    return {
        ...
        "future_office_candidates": [
            {
                ...
                "first_schema_contracts": [
                    {"schema_id": "ad_hypothesis_pack", "owner_agent": "zhongshu", "required_fields": ["target_audience", "core_selling_point", "platform", "assumption", "evidence_source"], "acceptance": "每条投放假设必须说明人群、卖点、平台和证据来源。"},
                    {"schema_id": "creative_script_matrix", "owner_agent": "bingbu", "required_fields": ["hook", "scene", "asset_reference", "cta", "risk_note"], "acceptance": "脚本矩阵必须能追到素材来源和投放风险。"},
                    {"schema_id": "material_usage_manifest", "owner_agent": "hubu", "required_fields": ["material_id", "source_office", "usage_right", "linked_script_ids"], "acceptance": "素材不能默认可投放，必须标明来源、授权边界和引用脚本。"},
                ],
                "first_recovery_events": [
                    {"stage": "creative_planning", "action": "regenerate_script_matrix", "preserves": ["product_research", "audience_decision"], "clears": ["creative_script_matrix", "ad_review_risk_report"], "user_message": "脚本跑偏时只重写脚本矩阵，保留已确认卖点和人群。"},
                    {"stage": "material_review", "action": "recheck_material_usage", "preserves": ["creative_script_matrix"], "clears": ["material_usage_manifest", "ad_review_risk_report"], "user_message": "素材授权或质量不清时重审素材，不重做投放假设。"},
                ],
            ...
            },
            {
                ...
                "first_no_key_sample": {
                    ...
                    "deliverables": ["选品决策报告", "来源清单", "竞品价格带", "评论痛点表", "供应链假设卡", "证据缺口卡", "阶段性交付声明"],
                    "acceptance": "用户能据此判断是否继续补证据、是否值得立项；不能把无来源数据当成真实平台结论。",
                },
                "launch_boundary": {
                    "status": "blocked_until_evidence",
                    "public_entry": "backlog_only",
                    "must_not_create": ["primary_hall_card", "real_task_executor", "shared_research_workspace"],
                    "first_allowed_work": "no-key sample and schema/recovery validators only",
                },
                "first_schema_outputs": ["selection_source_manifest", "selection_decision_report", "competitor_price_band", "review_pain_point_table", "supply_chain_hypothesis_cards", "evidence_gap_cards"],
                "first_schema_contracts": [
                    {"schema_id": "selection_source_manifest", "owner_agent": "shibu", "required_fields": ["source_id", "platform", "page_type", "capture_status", "source_or_gap", "allowed_claim"], "acceptance": "来源清单必须区分已截图、待补截图、公开资料和不可验证信息。"},
                    {"schema_id": "selection_decision_report", "owner_agent": "gongbu", "required_fields": ["category", "decision", "confidence", "key_reasons", "evidence_gap_ids", "boss_readable_summary"], "acceptance": "选品结论必须能回到证据缺口和关键原因，并给出老板可读的一页结论。"},
                    {"schema_id": "competitor_price_band", "owner_agent": "hubu", "required_fields": ["competitor", "platform", "price_low", "price_high", "sales_signal", "source_or_gap"], "acceptance": "价格带必须标明平台、价格区间、销量信号和来源；缺截图时写入待补证据。"},
                    {"schema_id": "review_pain_point_table", "owner_agent": "hubu", "required_fields": ["pain_point", "review_signal", "affected_user", "opportunity", "source_or_gap", "priority"], "acceptance": "评论痛点必须连接用户类型、机会点、优先级和来源或缺口。"},
                    {"schema_id": "supply_chain_hypothesis_cards", "owner_agent": "bingbu", "required_fields": ["hypothesis_id", "sourcing_assumption", "quality_risk", "moq_or_cost_unknown", "validation_step"], "acceptance": "供应链只能写成假设卡，必须标明待验证成本、起订量或质量风险。"},
                    {"schema_id": "evidence_gap_cards", "owner_agent": "xingbu", "required_fields": ["gap_id", "target_page", "why_needed", "suggested_file_name", "blocks_claim", "acceptance"], "acceptance": "每个待补证据都要说明页面、原因、命名、阻塞了哪条结论和验收标准。"},
                ],
                "first_recovery_events": [
                    {"stage": "platform_access", "action": "mark_platform_access_blocked", "preserves": ["category_scope", "requested_platforms"], "clears": ["selection_source_manifest", "public_claim_report"], "user_message": "平台或账号打不开时先生成待补证据清单，不把缺失数据伪装成真实结论。"},
                    {"stage": "evidence_capture", "action": "sync_selection_evidence", "preserves": ["category_scope", "uploaded_screenshots"], "clears": ["selection_source_manifest", "selection_decision_report", "competitor_price_band", "review_pain_point_table", "supply_chain_hypothesis_cards"], "user_message": "补充截图后只重跑证据识别和选品表格，不清空品类范围。"},
                    {"stage": "decision_packaging", "action": "rebuild_selection_pack", "preserves": ["category_scope", "selection_source_manifest", "competitor_price_band", "review_pain_point_table"], "clears": ["selection_decision_report", "public_claim_report"], "user_message": "结论不可信时重建选品决策包，保留已核验来源和表格。"},
                ],
                "human_review_points": ["确认品类范围", "确认目标平台", "确认待补截图", "确认供应链假设仅为假设", "确认阶段结论能否交给老板"],
                "forbidden_shortcuts": ["不能伪造飞瓜或电商后台数据", "不能把 staged demo 说成全自动采集", "不能复用研究办公室 workspace_id 写真实产物", "不能把供应链假设写成已验证结论"],
            },
            {
                "id": "story_ip",
                "name": "小说或短剧 IP 办公室",
                ...
                "first_schema_contracts": [
                    {"schema_id": "ip_adaptation_plan", "owner_agent": "zhongshu", "required_fields": ["source_story", "adaptation_goal", "core_conflict", "episode_count", "change_log"], "acceptance": "改编方案必须说明改什么、为什么改、和原故事的变化。"},
                    {"schema_id": "episode_hook_matrix", "owner_agent": "bingbu", "required_fields": ["episode", "hook", "turning_point", "viewer_question", "source_reference"], "acceptance": "每集钩子必须能回到原故事或已确认改编方向。"},
                    {"schema_id": "copyright_boundary_report", "owner_agent": "xingbu", "required_fields": ["source_material", "authorization_status", "risk", "allowed_public_claim"], "acceptance": "版权边界不清时必须阻止公开宣称可商用。"},
                ],
                "first_recovery_events": [
                    {"stage": "adaptation_planning", "action": "revise_ip_adaptation_plan", "preserves": ["source_story", "human_constraints"], "clears": ["ip_adaptation_plan", "episode_hook_matrix"], "user_message": "改编方向不对时保留原故事和用户约束，只重写企划。"},
                    {"stage": "rights_review", "action": "recheck_copyright_boundary", "preserves": ["ip_adaptation_plan"], "clears": ["copyright_boundary_report", "public_claim_report"], "user_message": "授权边界不清时重审版权声明，不继续扩展资产生产。"},
                ],
            ...
            },
            {
                ...
                "first_schema_contracts": [
                    {"schema_id": "technical_brief", "owner_agent": "zhongshu", "required_fields": ["goal", "scope", "constraints", "non_goals", "acceptance_tests"], "acceptance": "技术任务必须先锁范围、非目标和验收测试。"},
                    {"schema_id": "implementation_trace", "owner_agent": "libu", "required_fields": ["change_id", "file_path", "reason", "test_command", "rollback_note"], "acceptance": "实现记录必须能追到文件、原因、测试和回滚说明。"},
                    {"schema_id": "deployment_boundary_report", "owner_agent": "xingbu", "required_fields": ["environment", "secret_boundary", "online_claim", "verification_command"], "acceptance": "部署声明必须区分本地验证、CI 验证和线上验证。"},
                ],
                "first_recovery_events": [
                    {"stage": "implementation", "action": "rebuild_change_plan", "preserves": ["technical_brief", "user_constraints"], "clears": ["implementation_trace", "test_evidence_table"], "user_message": "实现路径不对时先重建变更计划，不自动提交代码。"},
                    {"stage": "test_evidence", "action": "rerun_test_evidence", "preserves": ["implementation_trace"], "clears": ["test_evidence_table", "deployment_boundary_report"], "user_message": "测试证据不足时重跑测试证据，不把本地通过说成线上通过。"},
                ],
        ...
        "cross_office_handoff_map": [
            {
                "id": "research_to_comic_production",
                "status": "manual_handoff_ready",
                "from_office": "research",
                "to_office": "comic_production",
                "user_scenario": "先用研究办公室形成市场洞察、用户痛点、竞品证据和机会判断，再交给 AI 漫剧制片办公室转成内容故事、资产和制片画布。",
                "handoff_artifacts": ["research_claim_report", "source_list", "data_table", "evidence_gap_cards", "opportunity_map"],
                "receiving_requirements": ["confirmed_story_input", "target_audience", "selling_points", "evidence_boundary"],
                "automation_level": "manual_copy_with_manifest",
                "not_automated_reason": "研究结论仍可能包含待补证据，自动转成故事会放大未验证结论；必须先由人确认哪些洞察可以进入创作。",
                "next_gate": "cross_office_handoff_schema",
            },
            {
                "id": "comic_production_to_short_video_ads",
                "status": "blocked_until_real_quality",
                "from_office": "comic_production",
                "to_office": "short_video_ads",
                "user_scenario": "把已确认的故事、资产引用链、镜头执行卡、提示词包和真实图片证据交给投放办公室，生成短视频脚本矩阵和素材使用清单。",
                "handoff_artifacts": ["word_canvas", "handoff_manifest", "asset_usage_map", "prompt_package", "production_acceptance"],
                "receiving_requirements": ["real_quality_verified", "material_usage_right", "platform_policy_review"],
                "automation_level": "blocked",
                "not_automated_reason": "当前公开样例仍是 fixture_only，不能把结构样例自动当作可投放真实素材。",
                "next_gate": "real_model_image_evidence",
            },
            {
                "id": "research_to_ecommerce_selection",
                "status": "backlog_first_candidate",
                "from_office": "research",
                "to_office": "ecommerce_selection",
                "user_scenario": "把研究办公室的来源、竞品表、截图计划和评论痛点交给电商选品办公室，形成选品决策包和阶段性交付声明。",
                "handoff_artifacts": ["research_evidence_manifest", "competitor_table", "review_pain_points", "screenshot_plan", "research_claim_report"],
                "receiving_requirements": ["selection_source_manifest", "evidence_gap_cards", "boss_readable_summary"],
                "automation_level": "backlog_only",
                "not_automated_reason": "电商选品办公室尚未拥有独立 schema、恢复事件、样例交付和公开声明边界。",
                "next_gate": "future_schema_validators",
            },
        ],
